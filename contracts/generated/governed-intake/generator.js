...
import { validateChecklistRelease,                    } from "./governed-intake-triage-state.migrate.js";
...
function assertTriageChecklistContract(raw                               ) {
  ...
  const semantics = (item                           )                          =>
    item?.semantics && typeof item.semantics === "object" && !Array.isArray(item.semantics)
      ? item.semantics                           
      : {};
  const scope = checklist.items.find((item               ) => item.id === scopeId);
  ...
  const verify = checklist.items.find((item               ) => item.id === "verify-human-required");
...
export function loadContract(root = REPO_ROOT) {
  ...
  const exploration = raw.workTypes.find((wt                ) => wt.id === "exploration");
  assert.equal(exploration?.recordKind, "governance/exploration");
  const experiment = raw.workTypes.find((wt                ) => wt.id === "experiment");
...
export function generateTaskMarkdown(contract) {
  ...
  return [
    ...
    "",
    "## Work type",
    `<!-- ${contract.workTypes.map((wt                   ) => wt.label).join(" | ")} -->`,
    "",
    `## ${workUnitKey.heading}`,
...
export function generateTaskYaml(contract) {
  ...
  return [
    ...
    "      options:",
    ...contract.workTypes.map((wt                   ) => `        - ${wt.label}`),
    "    validations:",
    "      required: true",
    "",
    "  - type: textarea",
