...
export function canonicalPolicyJson(value         )         {
  ...
  if (value && typeof value === "object" && Object.getPrototypeOf(value) === Object.prototype) {
    return `{${Object.keys(value).sort().map(key => `${JSON.stringify(key)}:${canonicalPolicyJson((value                           )[key])}`).join(",")}}`;
  }
  throw new TypeError("policy identity requires finite plain JSON");
}
const digest = (value         )         => `sha256:${createHash("sha256").update(canonicalPolicyJson(value), "utf8").digest("hex")}`;

/** File-integrity SHA-256 remains in the release manifest; these are semantic identities. */
export function bindTriagePolicy(policy              )                    {
  assertTriagePolicy(policy);
  const immutableInput = JSON.parse(canonicalPolicyJson(policy))                ;
  return { policy: immutableInput, policyIdentity: digest(immutableInput), rubricIdentity: digest(immutableInput.effortRubric) };
}
export function boundTriagePolicyFromProducer()                    {
  return bindTriagePolicy(policyJson                );
}
...
export function normalizeIssueScopeBody(body        )         {
...
export function fingerprintIssueScope(source                  )         {
